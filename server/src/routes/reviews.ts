...
router.post('/', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const hasDeliveredOrder = await Order.findOne({ userId, status: 'DELIVERED' })
    if (!hasDeliveredOrder) {
      const hasOrderWithVariant = await OrderItem.findOne({ variantId }).populate({ path: 'orderId', match: { userId, status: 'DELIVERED' } })
      if (!hasOrderWithVariant) return res.status(403).json({ success: false, message: 'You can only review products you have purchased and received' })
    }
