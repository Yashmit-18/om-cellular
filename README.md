...
- **Hosting targets**: Vercel (frontend, live), Render (backend, live), Netlify (legacy, unused) — CI workflow, `render.yaml` blueprint, explicit `client/vercel.json` (SPA rewrite, Vite config) and deployment config are present. **Verified 26 Sep 2026 (D35):** the Vercel frontend at `https://om-cellular.vercel.app` is live on branch `mern-migration` and byte-identical to a clean local production build of HEAD, and the Render backend reports the current commit from `/api/health`. The earlier "Vercel 404 / cut-over pending" state is resolved.
...
|---|---|---|---|
| Frontend | Vercel | `client/vercel.json` and SPA rewrite `/(.*) → /index.html` | **✅ LIVE — `https://om-cellular.vercel.app` serving `mern-migration`.** All 97 JS chunks plus `index.html` and CSS are SHA256-identical to a clean local production build of HEAD. The earlier "returns 404" state is resolved. |
| Frontend (alt) | Netlify | `netlify.toml` (base `client`, build `npm run build`, publish `dist`, `VITE_API_URL=https://om-cellular.onrender.com/api/v1`) | **Legacy / not in use.** No active deployment: `om-cellular.netlify.app` returns 404, and there is no Netlify GitHub integration, `.netlify/` state, `netlify.json` or deploy workflow in the repo. Retained only because Netlify site/integration state is external to the repository and cannot be verified from here. See §D35. |
| Backend | Render | `render.yaml` blueprint added (web service, build `npm ci && npm run build`, start `npm start`) + dashboard/env | **✅ LIVE — serving `a624d2c`.** `/api/health` reports the commit; public catalog/search/valuation/serviceability and public order/repair tracking return 200; admin-only endpoints correctly 401 without auth. |
...
Production frontend: **`https://om-cellular.vercel.app`** · Production backend: **`https://om-cellular.onrender.com`** · Deployed branch: **`mern-migration`**.

Credentialed CORS origins (from `server/src/config/cors.ts`): `https://om-cellular.vercel.app`, plus `env.CLIENT_URL`, `env.CLIENT_ORIGINS` (comma-separated) and `localhost:5173/3000` for development. Origins are matched by exact string equality — no wildcards, suffixes or patterns. The secondary origin `https://om-cellular-iota.vercel.app` was **removed in D35**; it was a leftover from the reverted root-`vercel.json` deployment attempt and is referenced by no client code, canonical link, sitemap entry or deployment config. Because production auth cookies are `SameSite=None`, any allow-listed origin is a host the browser will attach auth tokens to, so unowned hosts must not be listed.

✅ **Frontend/backend cut-over is complete (verified 26 Sep 2026 — D35).** The Vercel frontend at `https://om-cellular.vercel.app` serves `mern-migration` and was proven byte-identical to a clean local production build of HEAD (97/97 JS chunks, `index.html` and CSS all SHA256-matched), and the Render backend serves the current commit. The prior "Vercel returns 404" gap recorded in earlier phase notes no longer applies.
...
- **Frontend build**: `client` → `tsc -b && vite build` → `dist/`; `client/vercel.json` is explicit (`framework: vite`, build + `outputDirectory: dist`, SPA rewrite). The old **root `vercel.json` was removed** — it pointed Vercel at a non-existent root `package.json` (the cause of the silent 404). The root `netlify.toml` is **legacy and unused** (no active Netlify site; kept only because Netlify site/integration state is external to this repo). Vite dev proxies `/api` → `localhost:5000`, and `VITE_API_URL` is inlined at build time.
...
- **Env (client)**: `VITE_API_URL` (`client/.env.example`). Vercel sets `https://om-cellular.onrender.com/api/v1` at build time. CI (`.github/workflows/ci.yml`) sets the same value on the `client` job and asserts it is inlined into the built entry chunk, so a validated artifact always targets the production API instead of the relative `/api/v1` fallback. The legacy `netlify.toml` also sets it.
...
- Production deploy state (updated 26 Sep 2026, D35): **Vercel = 🟢 live** at `https://om-cellular.vercel.app` on branch `mern-migration` (byte-identical to a clean local production build of HEAD); **Render = 🟢 live** on the current commit, versioned health verified. The former "Vercel cut-over pending" state is resolved — no dashboard rebind is outstanding. **Netlify = ⚪ legacy**, no active site (`om-cellular.netlify.app` 404), retained in-repo only because Netlify site/integration state is external and unverifiable from here. Deployment state is reported only from observed evidence; no CLI/dashboard access means nothing about a deploy is ever assumed.
...
| Production API (live Render, re-verified 26 Sep 2026) | ✅ **200 — serving the current commit `a624d2c`**: `/api/health` returns the deployed commit matching HEAD; `/api/v1/serviceability/check` (all 5 services), `/api/v1/phone-valuations/calculate`, `/api/v1/products`, search, brands/categories/settings, repair-services, phone-catalog, public order + repair tracking all verified live. Admin-only endpoints correctly 401 without auth. |

---

## 25. Known Issues (verified)

1. **The frontend cut-over is complete.** The deploy config is correct (explicit `client/vercel.json` + removed root `vercel.json`; `render.yaml` health check; CI present with `VITE_API_URL` asserted). `https://om-cellular.vercel.app` serves `mern-migration` and was verified byte-identical to a clean local production build of HEAD, so the previously-listed "Vercel 404" issue is closed. Residual items: the exact Vercel *deployment commit* cannot be proven from public artifacts (see §D35), and the legacy `netlify.toml` remains in-repo with external-only Netlify state.
...
| Production Deployment | 🟢 | 80% | Deploy config correct and **verified live end to end**: Vercel frontend at `https://om-cellular.vercel.app` serves `mern-migration` byte-identical to a clean local build of HEAD, Render backend serves the current commit with a versioned health check, CI asserts the production `VITE_API_URL`, and **safe CMS + coupon seeds run live**. **Remaining: online payments unprovisioned; the exact Vercel deployment commit is not provable from public artifacts; legacy `netlify.toml` retained with external-only state.** |
...
1. ~~Finish the frontend cut-over so `mern-migration` is what renders on Vercel.~~ **Done (D35, 26 Sep 2026).** `https://om-cellular.vercel.app` renders `mern-migration`; all 97 JS chunks plus `index.html`/CSS are SHA256-identical to a clean local production build of HEAD, and browser regression confirmed homepage, catalog, search, product detail, add-to-cart, cart and the checkout auth gate all render with no failed requests. *Superseded by:* the remaining top lever is online payment provisioning.
...
Backend live:             YES   (Render verified 26 Sep 2026 on commit a624d2c — health, serviceability, valuation, catalog, tracking all 200)
Frontend live:            YES   (Vercel verified 26 Sep 2026 — om-cellular.vercel.app serves mern-migration, byte-identical to a clean local build of HEAD)
Business-ready:           PARTIAL (online payments disabled — external Razorpay provisioning;
...
- **What it is not yet:** production-provisioned commerce end to end. Payments are unprovisioned (so live pay/refund is code-complete but unexercised) — this is now the leading gap, and maturity features (delivery providers, courier integration, exchange instant credit, notification delivery, guest checkout, SEO prerender, native apps) are ahead. The Vercel frontend is live and no longer a gap.
- **Single biggest lever:** provision Razorpay and run one live pay/refund — online payments are the main remaining gap. The frontend cut-over that previously led this list is **done** (`https://om-cellular.vercel.app` serves `mern-migration`, verified byte-identical to a clean local build) and the Render backend is on the current commit, so the remaining gaps are incremental rather than architectural.

---

## D35 — Production Security Hardening, Deployment Config Cleanup & CI Reproducibility (26 Sep 2026)

Read-only-first audit of production architecture and deployment configuration, followed by narrowly scoped fixes for four verified findings. No storefront, checkout, idempotency or authentication behaviour was changed.

### D35.1 Credentialed CORS allow-list — stale secondary origin removed

`https://om-cellular-iota.vercel.app` was removed from the production origin allow-list in `server/src/config/cors.ts`.

**How it was classified before changing anything:** stale, not required. Evidence gathered:

- It was introduced in `7bf92a9` ("Fix Vercel SPA routing and frontend deployment") *together with a root `vercel.json`*, as part of an attempt to fix the old Vercel 404. That root `vercel.json` was deleted in `ccd1e40`, which moved the frontend to `client/vercel.json` + `om-cellular.vercel.app`. The CORS entry was orphaned at that point and never revisited.
- **Zero client references.** `client/index.html` (canonical, Open Graph, Twitter), `client/public/sitemap.xml` (all 9 URLs), `client/public/robots.txt` and `client/src/pages/shop/ProductDetailPage.tsx` all reference only `om-cellular.vercel.app`.
- No deployment config targets it: not `client/vercel.json`, not `netlify.toml` (which never mentioned it), not `.github/workflows/ci.yml`, not `render.yaml`.
- `server/.env.example` documents `CLIENT_URL=https://om-cellular.vercel.app`.
- Live host returns HTTP 404 `text/plain` with `Server: Vercel` — it serves no storefront.

**Why it mattered:** production auth cookies are `SameSite=None` + `Secure` + `httpOnly` (`server/src/routes/auth.ts`), so every allow-listed origin is a host the browser will attach `accessToken`/`refreshToken` to. Because `*.vercel.app` resolves by wildcard DNS, an unclaimed project name can be registered by a third party, who would then be able to make credentialed API calls — including `/auth/refresh` — as a logged-in victim and read the responses.

**Preserved unchanged:** canonical origin `https://om-cellular.vercel.app`, `credentials: true`, `Content-Type`, `Authorization`, `X-Requested-With`, `Idempotency-Key`, all HTTP methods, `Vary: Origin`, and the `env.CLIENT_URL` / `env.CLIENT_ORIGINS` / localhost development entries. No `*` was introduced; origins still match by exact string equality, with no suffix, prefix or regex rules.

`server/tests/cors.test.ts` grew from 11 to 19 tests and now pins: canonical origin allowed; `om-cellular-iota.vercel.app` rejected; `om-cellular.vercel.app.evil.com` rejected; other prefix/suffix/subdomain look-alikes rejected; `Origin: null` never reflected; no non-canonical `*.vercel.app` origin reflected; and the `Idempotency-Key` preflight still returning 204 for the canonical origin.

### D35.2 Netlify — legacy, left in place with evidence

`netlify.toml` is **not** deleted. Evidence of no active deployment: `om-cellular.netlify.app` returns 404; there is no `.netlify/` state, `netlify.json`, `netlify/functions` or Netlify deploy workflow in the repository; and the canonical production frontend is Vercel, verified live. Netlify *site* and *GitHub-integration* state live in the Netlify dashboard and cannot be verified from this repository, so the file is retained as documented legacy configuration rather than deleted on inference. Recommended owner action: disconnect the Netlify site (or leave it and accept the residual risk of a default-branch publish of the older `main` tree).

### D35.3 CI reproducibility — production `VITE_API_URL` asserted

`.github/workflows/ci.yml` now sets `VITE_API_URL: https://om-cellular.onrender.com/api/v1` on the `client` job and adds a post-build step that asserts the URL is inlined in `dist/assets/index-*.js`. This value is a public, non-secret URL already committed in `netlify.toml` and `client/.env.example`; it is deliberately not a repository secret. The check matters because `client/src/services/api.ts` falls back to the relative `/api/v1` when the variable is unset, and the Vercel static origin has no `/api` proxy, so such an artifact would 404 on every request. CI remains validation-only and still does not deploy.

### D35.4 Documentation

This README's deployment status, production URLs, deployed branch and backend baseline were corrected where demonstrably stale (the "Vercel returns 404 / cut-over pending" claims, the `ccd1e40` baseline, and the inaccurate statement that `netlify.toml` referenced the secondary origin). Unrelated documentation was left untouched.

### D35.5 Known limitation — frontend deployment commit is not provable

The deployed frontend is **proven** to be the current client source (97/97 JS chunks plus `index.html` and CSS are SHA256-identical to a clean local production build of HEAD), but the **exact deployment commit cannot be established**. `client/` has had no changes since `89cbd07`, so the deployed bytes are equally consistent with any commit from `89cbd07` through HEAD. Public Vercel metadata does not help: `X-Vercel-Id` is a request routing/region ID, not a deployment ID, and per-asset `Last-Modified` values are staggered cache-population times rather than an atomic deploy timestamp. Resolving this requires Vercel dashboard or API access, which is unavailable from this machine. No deployment is claimed without evidence.

---

*Key file anchors: client routing `client/src/App.tsx`; layout guards `client/src/layouts/{AccountLayout,AdminLayout}.tsx`; api layer `client/src/services/api.ts`; payment client `client/src/pages/shop/CheckoutPage.tsx` + `client/src/services/payment.service.ts`; server entry `server/src/app.ts`; auth `server/src/routes/auth.ts` + `server/src/middleware/auth.ts`; CORS policy `server/src/config/cors.ts` + `server/tests/cors.test.ts`; serviceability `server/src/services/serviceabilityLogic.ts` + `serviceability.service.ts`; returns `server/src/routes/returns.ts` + `server/src/models/returnRequest.model.ts`; warranty `server/src/routes/warranties.ts` + `server/src/services/warranty.service.ts`; inventory ledger `server/src/routes/inventory.ts` + `server/src/models/inventoryLedger.model.ts`; coupons `server/src/services/coupon.service.ts`; sell/exchange completion `server/src/utils/requestValidation.ts` (inspection checklist + payout + auto-payout); payments `server/src/routes/payments.ts`; analytics `server/src/routes/analytics.ts`; database config `server/src/config/database.ts`; env `server/src/config/env.ts` + both `.env.example`; deployment config `client/vercel.json`, `netlify.toml` (legacy), `render.yaml`, `.github/workflows/ci.yml`.*
