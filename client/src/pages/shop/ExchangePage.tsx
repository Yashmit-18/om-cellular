  ...
  return (
    <div className="bg-gradient-to-b from-ivory-100/60 via-white to-white">
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <form onSubmit={handleSubmit} className="card mt-8 space-y-6 p-8">
          {/* Old Phone Section */}
          <div>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <div className="relative" ref={modelRef}>
                ...
                <button type="button" onClick={() => setShowModelDropdown(!showModelDropdown)} disabled={!selectedBrand || loadingModels}
                  className="input mt-1 flex items-center justify-between text-left disabled:cursor-not-allowed disabled:opacity-50">
                  <span className={selectedModel ? '' : 'text-gray-400'}>{loadingModels ? 'Loading models...' : (modelSearch || 'Select model...')}</span>
                  <ChevronDown className="h-4 w-4 shrink-0 text-gray-400" />
                </button>
                {showModelDropdown && (
                  <div className="absolute z-20 mt-1 w-full rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
                    ...
                    <div className="max-h-48 overflow-y-auto">
                      {loadingModels ? (
                        <div className="space-y-2 p-3">
                          {[0, 1, 2].map(n => <div key={n} className="skeleton h-9 w-full" />)}
                        </div>
                        ...
                        <button key={(m as any).id || (m as any)._id} type="button" onClick={() => { setSelectedModel(m); setModelSearch(m.modelName); setShowModelDropdown(false) }} className="flex min-h-[44px] w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-gray-50">
                          {m.image
                            ? <img src={m.image} alt="" className="h-8 w-8 shrink-0 rounded-lg object-contain" onError={e => { (e.target as HTMLImageElement).style.display = 'none' }} />
                            : <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-100"><Smartphone className="h-4 w-4 text-gray-400" /></div>}
                          <span className="truncate">{m.modelName}</span>
                        </button>
                      )) : (
                        <p className="px-3 py-3 text-xs text-gray-500">{selectedBrand ? 'No models found for this brand' : 'Select a brand first'}</p>
                      )}
                    </div>
                  </div>
                )}
