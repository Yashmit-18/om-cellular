...
export default function AdminCouponsPage() {
  ...
  const emptyForm = { code: '', description: '', type: 'PERCENTAGE', value: 0, minOrderAmount: 0, maxDiscount: 0, usageLimit: 0, expiresAt: '', isActive: true }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        ...
        setCoupons(coupons.map(c => (c.id === editingId ? res.data.data : c)))
  ...
  const handleEdit = (coupon: any) => {
    ...
    setForm({
      ...
      value: coupon.value || 0, minOrderAmount: coupon.minOrderAmount || 0, maxDiscount: coupon.maxDiscount || 0,
      usageLimit: coupon.usageLimit || 0, expiresAt: coupon.expiresAt ? String(coupon.expiresAt).slice(0, 10) : '', isActive: coupon.isActive ?? true,
    })
  }
    ...
    <div>
      ...
      <div className="mt-6 card p-6">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          ...
          <div className="md:col-span-3"><input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Description (shown at checkout)" className="input" /></div>
        ...
        <button onClick={handleSubmit} className="btn-primary mt-4"><Plus className="mr-1 h-4 w-4" /> {editingId ? 'Save Changes' : 'Add Coupon'}</button>
      </div>
            ...
            <tr key={c.id} className="border-b last:border-0 hover:bg-gray-50">
              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-0.5">
                  <button onClick={() => handleEdit(c)} className="text-gray-400 hover:text-brand-600"><Pencil className="h-4 w-4" /></button>
