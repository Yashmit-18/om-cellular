...
const STATUS_OPTIONS = ['SUBMITTED', 'UNDER_REVIEW', 'APPROVED', 'INSPECTED', 'OFFER_MADE', 'OFFER_ACCEPTED', 'OFFER_DECLINED', 'REJECTED', 'PICKUP_SCHEDULED', 'PICKED_UP', 'PAYMENT_PENDING', 'PAYMENT_COMPLETED', 'CANCELLED']
...
export default function AdminSellDetailPage() {
  ...
  useEffect(() => {
    if (!id) return
    api.get(`/sell-requests/${id}`).then(r => {
  ...
  const handleStatusUpdate = async (status: string) => {
    ...
    if (finalPrice && finalPrice !== '' && (status === 'APPROVED' || status === 'UNDER_REVIEW')) data.finalOfferedPrice = Number(finalPrice)
    if (status === 'INSPECTED') {
    ...
    if (status === 'PAYMENT_PENDING' || status === 'PAYMENT_COMPLETED') {
      if (!payout.amount || Number(payout.amount) < 0) {
        toast.error('Enter a payout amount')
        return
      }
      data.payout = { amount: Number(payout.amount), mode: payout.mode, reference: payout.reference.trim() || undefined, status: status === 'PAYMENT_COMPLETED' ? (payout.status === 'PAID' ? 'PAID' : 'PENDING') : 'PENDING' }
    }
    setUpdating(true)
    try {
      const res = await api.put(`/sell-requests/${id}`, data)
  ...
  const handleMarkPaid = async () => {
    ...
    setUpdating(true)
    try {
      const res = await api.put(`/sell-requests/${id}`, {
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="space-y-4">
          <div className="card p-6 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><p className="text-gray-500">Device</p><p className="font-medium">{request.brand} {request.model}</p></div>
            ...
            {request.imei && <div className="rounded-lg bg-gray-50 p-3 text-sm"><p className="text-xs font-semibold text-gray-500 uppercase">IMEI</p><p className="mt-0.5 font-mono">{request.imei}</p></div>}
            {request.deviceDetails && <div><p className="text-gray-500 text-sm">Device Notes</p><p className="mt-1 text-sm">{request.deviceDetails}</p></div>}
            ...
            {pickup && (
              <div className="rounded-lg bg-gray-50 p-3 text-sm">
                ...
                <p className="mt-1">{pickup.name}, {pickup.phone}</p>
                <p>{pickup.addressLine1}{pickup.landmark ? ` (${pickup.landmark})` : ''}</p>
          ...
          {request.payout && (
            <div className="card p-6">
              ...
              <p className="mb-3 text-xs text-gray-400">Disbursement record for this sell deal.</p>
              <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="card p-6">
            <h2 className="font-semibold mb-3">Request Timeline</h2>
        ...
        <div className="space-y-4">
          <div className="card p-6">
            ...
            <div className="mb-3"><label className="block text-xs font-medium text-gray-600">Final Offer Price (₹, optional)</label><input value={finalPrice} onChange={e => setFinalPrice(e.target.value)} type="number" className="input mt-1 !py-2 text-sm" placeholder="e.g. 12000" /></div>
            ...
            <div className="mb-4 rounded-lg border border-gray-200 p-3">
              <p className="mb-2 text-xs font-medium text-gray-600">Payout (used at PAYMENT_PENDING / PAYMENT_COMPLETED)</p>
              <div className="grid gap-2 sm:grid-cols-2">
