...
| D12 | Catalog expansion (real tablets/smartwatches/accessories, idempotent seed, category-aware icon fallback) + storefront filter unification so the Price filter is visible on `/buy-phones` (`ProductCatalog` shared by `BuyPhonesPage` + `ProductsPage`; generalized `productCatalog` builder + catalog-model schema fields; `catalogExpansion` seed + 10 unit tests) + **committed & pushed to origin/mern-migration** | ✅ done |
